...
def start_video_object_detection(video: str, net, out_layers, classes, classes_to_look_for):
    """
    ...
    while True:
        try:
            ...
            while video_camera_capture.isOpened():
                ...
                frame = apply_yolo_object_detection(frame, net, out_layers, classes, classes_to_look_for)

                # Displaying the processed image on the screen with a reduced window size
                frame = cv2.resize(frame, (1980 // 2, 1280 // 2))
...
def main():
    if __name__ == '__main__':
        ...
        # The names are in the file coco.names


        video = "rtsp://91.77.164.58:554/11"
        look_for = ['car','bus','person']

        classes_to_look_for = look_for

        start_video_object_detection(video, net, out_layers, classes, classes_to_look_for)
